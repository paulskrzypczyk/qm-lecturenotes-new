...
We can understand this from a second perspective. Our requirement to have a definite wavelength means that a particle has to be spread out **everywhere**. If this were possible, then what would be probability density $\pd(x)$ to find the particle at some location $x$? This would be
```{math}
\vph \pd(x) = |v_p(x)|^2 &= \left| \frac{1}{\sqrt{2\pi \hbar}}e^{ipx/\hbar}\right|^2, \\
...
&= \tilde{\psi}(p'),
```
where to obtain the second line we have used the normalisation condition [](#e-momentum-norm). What this shows is a very important relation:
````{card}
```{math}
:label: e-p-scalar
