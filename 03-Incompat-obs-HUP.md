...
We can illustrate the main idea behind the incompatibility of position and momentum by considering a simple example — that of a **gaussian wavefunction**. Consider the following family of wavefunctions 
```{math}
...
What is remarkable is that what we see above **isn't just a special property of Gaussian wavefunctions**, but is a general fact about quantum mechanics. In fact, 
...
This shows that if we have a particle in a particular quantum state $\ket{\psi}$ with corresponding spatial wavefunction $\psi(x)$, and we calculate the uncertainty in the position of the particle $\Delta x$, then even before calculating the momentum wavefunction, we know for certain that $\Delta p \geq \hbar/2\Delta x$. Hence if $\Delta x$ is small, then this implies that $\Delta p$ is necessarily going to be large. 
...
The question we will answer in the next section is **how to determine whether two observables are incompatible or not in quantum mechanics?** We will see that this is determined to a purely **mathematical** proporty of the **operators** associated to each observable property, known as **commutation**:
...
```
````
That is, the commutator is the **difference** between the two orders. Having introduce the commutator, we can re-state in a more succinct manner what is means for two operators to commute: 
> $\hat{A}$ and $\hat{B}$ commute if (and only if) $[\hat{A}, \hat{B}] = 0$. 

This is because the commutator will vanish if $\hat{A}\hat{B} = \hat{B}\hat{A}$, i.e. precisely what the two different orders coincide. 
```{aside}
We note that there is something that is a little strange here, and can be misleading. While commutation is about the **order** of operators, this doesn't imply that imcompatibility of observables is **also** about order. This is a common misconception. The link is in fact a very mathematical one: commutation is a succinct way of determining whether two operators have common **eigenstates** (i.e. whether the eigenstates of one operator happen to coincide with the eigenstates of another operator or not). It is this property which is crucial for incompatibility, and it happens that this will be the case only when operators commute. 
```
...
```
````
Note that, in this case, we really **need** the wavefunction in order for this to make sense properly, even though the wavefunction is **completely arbitrary**. 

Crucially, the commutation of any two operators $\hat{A}$ and $\hat{B}$ can be determined from $\op{A}$ and $\op{B}$. To explain what this means, we will use the example of $\hat{X}$ and $\hat{P}$. As an exercise, you will show that
```{math}
